  ...
  Future<void> _loadUserProfile() async {
    try {
      ...
      if (userId != null) {
        ...
        if (user != null) {
          if (mounted) {
            print('DEBUG: Usuario cargado: ${user.fullName}, Avatar: ${user.avatarUrl}');
            setState(() {
              _user = user;
          ...
          final fullName = currentUser?.userMetadata?['full_name'] ?? email.split('@')[0];
          
          // Crear registro en BD
          await Supabase.instance.client.from('users').insert({
            'id': userId,
            'auth_id': userId,  // Necesario para RLS policies
            'email': email,
            'full_name': fullName,
            'role': 'member',
            'status': 'active',
